...
class ReviewPreprocessor:
    ...
    def clean_text(self):
        ...
        before_eng = len(self.df)
        self.df = self.df[self.df['review_text'].apply(is_english)]
        removed_eng = before_eng - len(self.df)
        print(f"Removed {removed_eng} non-English reviews")
        self.stats['non_english_removed'] = removed_eng
    ...
    def generate_report(self):
        ...
        if self.df is not None:
            ...
            print(f"Text length: avg {self.df['text_length'].mean():.0f}, median {self.df['text_length'].median():.0f}")
